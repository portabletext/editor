...
The `renderListItem` render prop on `<PortableTextEditable>` is removed, along with the `RenderListItemFunction` type and the `data-list-index` attribute on legacy-rendered text blocks (`data-list-item` and `data-level` stay). Core no longer computes list-item indices at all. The `BlockListItemRenderProps` type remains exported but is deprecated.
