...
{/* While some actions share names with events, it's important to remember that actions don't send events. */}
...
The core selectors are useful helper functions for checking conditions of the editor, finding selected text, and more.
