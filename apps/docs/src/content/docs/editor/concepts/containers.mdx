...
Registering nodes with `NodePlugin` hands the position, and everything rendered inside it, to registration-based rendering, and that changes who owns what. Containers, text blocks, and block objects each start a rendering subtree: your `render` owns the outer wrapper entirely, and the engine emits only `data-pt-*` attributes inside it. A standalone inline-object or span registration doesn't start a subtree on its own. It renders in whichever mode its enclosing text block is already in: registration-based when the text block is registered, the deprecated render props' attribute shape when it is not. Span-level rendering keeps composing: `renderDecorator`, `renderAnnotation`, `renderPlaceholder`, and range decorations fire on the spans inside `children` regardless of who renders the block.
