...
That ownership covers everything inside the node, a rendering subtree, and anything unregistered inside it renders through the engine defaults. So in practice a container travels with company: a `defineTextBlock` for the paragraphs inside it, and a registration for anything else that renders there:
