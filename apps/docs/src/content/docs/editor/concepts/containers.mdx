...
Node rendering happens entirely through registrations: the node render props that used to live on `<PortableTextEditable>` are gone. Registering nodes with `NodePlugin` opts those positions, and everything rendered inside them, into the editor's render pipeline, and that changes who owns what. Your `render` owns the outer wrapper entirely: the engine emits only `data-pt-*` attributes. Span-level rendering keeps composing: `renderDecorator`, `renderAnnotation`, `renderPlaceholder`, and range decorations fire on the spans inside `children` regardless of who renders the block.
...
If your editor still renders through the v7 render props on `<PortableTextEditable>` (`renderBlock`, `renderChild`), migrate before upgrading: each has a registration equivalent, and the engine dispatched per `_type`, so you could migrate one kind at a time. The [migration guide](/editor/guides/migrate-render-props/) walks through it.
:::
