...
Registering nodes with `NodePlugin` hands the position, and everything rendered inside it, to registration-based rendering, and that changes who owns what. Containers, text blocks, and block objects each start a rendering subtree: your `render` owns the outer wrapper entirely, and the engine emits only `data-pt-*` attributes inside it. A standalone inline-object or span registration doesn't start a subtree on its own, it renders in whichever mode its enclosing text block is already in, registration-based if the text block is registered too, the deprecated render props' attribute shape otherwise. Span-level rendering keeps composing: `renderDecorator`, `renderAnnotation`, `renderPlaceholder`, and range decorations fire on the spans inside `children` regardless of who renders the block.

The pipeline boundary is the subtree. Inside a registered container, unregistered node types render through the engine defaults, so registering a container usually means bringing a `defineTextBlock` (and registrations for anything else that renders inside it) along with it.
...
function TextBlock(props: TextBlockRenderProps) {
  ...
  return (
    <div {...props.attributes} style={{position: 'relative'}}>
      ...
      {props.children}
      {/* DropIndicator: your own component, see the plugin-dnd README below */}
...
`snapshot.context.containers` exposes what's currently registered, keyed by the container's bare block-object `_type` (`callout`, `table`, and so on). Each entry carries `type`, the `field` that holds the container's editable children (its name, plus the schema's `of` list of allowed child types), and, if the registration declared positional overrides, a nested `of` array of further container or positional (span, block object, inline object) entries. Only top-level registrations appear as flat map entries: a type registered solely inside a parent's `of` shows up nested there, not as its own entry.
...
const nodes = [
  ...
  defineBlockObject({
    ...
    render: ({attributes, children, node, readOnly}) => (
      <figure {...attributes}>
        <div contentEditable={false} draggable={!readOnly}>
          <img src={String(node.src)} alt="" style={{width: '100%'}} />
        </div>
        {children}
      </figure>
    ),
  ...
  defineContainer({
    ...
    of: [
      defineBlockObject({
        ...
        render: ({attributes, children, node, readOnly}) => (
          <span {...attributes}>
            <span contentEditable={false} draggable={!readOnly}>
              <img src={String(node.src)} alt="" style={{height: '4rem'}} />
            </span>
            {children}
          </span>
        ),
