...
Mount both through `BehaviorPlugin`, the same way as any other custom Behavior. `plugin-table`'s table copy handler is a worked example of a `serialize.data` handler that produces `text/markdown` on its own, since the core converter is gone for it too.
