...
`renderDecorator` and `renderAnnotation` both carried `editorElementRef`, a ref to the engine-owned leaf DOM anchor. No registration exposes it, and the prop is gone, so there is nothing to fall back to: put a ref on your own rendered element instead. A decorator's `render` owns the markup and an annotation's composes inside the engine's anchor, so a ref on your own element reaches the same position in the DOM. Both props also carried `schemaType` (`renderAnnotation` carried `block` too), which a registration's `render` doesn't receive, but you can derive them yourself from a `path`: `getPathSubSchema` and `getEnclosingBlock`, both from the [traversal API](/editor/concepts/traversal/), read the sub-schema and enclosing text block inside a selector.
:::
...
Unrelated to render props, but also a v8 change: the editor no longer ships a built-in `text/markdown` converter. Pasting Markdown-formatted text no longer auto-parses it into Portable Text blocks, and copying no longer writes a `text/markdown` entry to the clipboard alongside `text/html` and `text/plain`.

To restore the old behavior, add [`@portabletext/markdown`](https://github.com/portabletext/editor/tree/main/packages/markdown) as your own dependency and register two Behaviors:
...
Mount both through `BehaviorPlugin`, the same way as any other custom Behavior. `event.blocks` is set when an upstream Behavior narrows what gets serialized, `plugin-table`'s table copy handler does this for a rectangular cell selection, so preferring it over `getFragment(snapshot)` also gets that rectangle serialized to Markdown for free once this Behavior is mounted.
