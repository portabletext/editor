...
Selectors are pure functions that derive state from the editor snapshot. They provide a clean way to access editor state without directly parsing the snapshot structure. Selectors delegate their path work to the [traversal API](/editor/concepts/traversal/).
