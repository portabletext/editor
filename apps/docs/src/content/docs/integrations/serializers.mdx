...
Most renderers use `types` and `marks` (plural) for custom components. **Astro uses `type` and `mark` (singular).** Check the examples below for your framework.
:::
...
<Tabs>
  <TabItem label="React">
    ...
    That's it for basic rendering. The component handles paragraphs, headings, bold, italic, links, and lists out of the box.
