...
Portable Text is stored as JSON. To display it, you pass the data through a **serializer** that converts each block into your target output: HTML strings, React components, Vue templates, or any other format.

## How it works

A Portable Text document is an array of blocks. A serializer walks the array and handles each block based on its `_type`:

1. **Text blocks** (`_type: "block"`) contain paragraphs, headings, lists, and inline formatting. Serializers handle these out of the box.
2. **Custom blocks** (any other `_type`) represent structured content like images, code blocks, or calls to action. You provide a component for each custom type.
3. **Marks** add meaning to inline text. **Decorators** (bold, italic, underline) work by default. **Annotations** (links, references, footnotes) carry data and need custom components.
...
That's enough to render paragraphs, headings, lists, bold, italic, and links. The next sections cover custom types and marks.

## Rendering custom block types

Custom blocks carry structured data that the serializer doesn't know how to render by default. You tell it what to do by mapping the block's `_type` to a component.
