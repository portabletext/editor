...
`table` uses `DefaultTableRenderer` automatically; pass `types: {table: undefined}` to opt out and fall back to a JSON code block instead.
...
\* Requires registering the built-in renderer (see above). Tables render by default; pass `types: {table: undefined}` to opt out.
