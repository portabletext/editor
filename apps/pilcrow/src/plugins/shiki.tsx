 ...
 *
 * Shiki emits a 2-D array of tokens (`ThemedToken[][]`), one inner
 * array per source line. Each token has `content`, an optional
 * `color`, and an `offset` relative to the original input. This
 * function distributes the tokens back across the corresponding line
 * blocks in pilcrow's `code-block` container so each token becomes a
 * decoration over the matching text span.
 *
 * Tokens without a color or with empty content are skipped (nothing
 * to render).
 *
 * Tokens whose line index has no corresponding line key are dropped
 * (parser produced extra trailing lines).
 */
...
/**
 * Returns the current Shiki `BundledLanguage` for the given pilcrow
 * code-block language string. Unknown languages map to `text` (no
 * highlighting).
 */
...
/**
 * Computes Shiki decorations for every `code-block` in the current
 * editor value. Tokenization is async; while a fresh run is in
 * flight the hook returns the previous decorations so the editor
 * does not flash uncoloured.
 *
 * Cached per `(language, source)` pair so unrelated edits don't
 * trigger re-tokenization.
 */
export function useShikiDecorations(
  theme: 'light' | 'dark',
