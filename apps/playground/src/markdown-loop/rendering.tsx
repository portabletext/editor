...
function MarkdownLoopTextBlock(props: TextBlockRenderProps): JSX.Element {
  ...
  const rawAlignment = (props.node as {alignment?: string}).alignment
  const alignment =
    rawAlignment === 'center' || rawAlignment === 'right'
      ? rawAlignment
      : undefined
  ...
  return (
    <div {...props.attributes} className={textBlockStyle({alignment})}>
...
function MarkdownLoopBlockObject(props: BlockObjectRenderProps): JSX.Element {
  if (
    props.node._type === 'horizontal-rule' ||
    props.node._type === 'sectionDivider'
  ) {
    return (
  ...
  if (props.node._type === 'code' || props.node._type === 'codeBlock') {
