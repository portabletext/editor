...
import type {DecorationLayer} from '@portabletext/plugin-decorations'
import {
  assertEvent,
  assign,
  emit,
...
export type DecorationLayerKind = 'comments' | 'presence'
...
export const playgroundMachine = setup({
  types: {
    context: {} as {
      ...
      layers: Record<
        EditorActorRef['id'],
        Partial<Record<DecorationLayerKind, DecorationLayer>>
      >
    },
    events: {} as
      | {type: 'add editor'}
      ...
      | {type: 'reactivate comment'; id: string; text: string}
      ...
      comments: ({context, event}) => {
        ...
        return context.comments.map((comment) =>
          comment.id === event.id && comment.status === 'orphaned'
            ? {...comment, status: 'active' as const, currentText: event.text}
            : comment,
        )
      },
    }),
