Feature: Insert Blocks
  ...
  Scenario Outline: Inserting blocks on an empty editor
    ...
    And "P: foo;;P: bar" is inserted at <placement> and selected at the <selection>
    And "baz" is typed
  ...
  Scenario Outline: Inserting blocks on a block object
    ...
    And "P: foo;;{BREAK};;P: bar" is inserted at "auto"
    Then the text is <text>
  ...
  Scenario: Inserting text blocks on a block object
    When "{IMAGE}" is inserted at "auto" and selected at the "end"
    And "P: foo;;P: bar" is inserted at "auto"
    Then the text is <text>
  ...
  Scenario Outline: Inserting blocks on a text block
    ...
    And "P: bar;;{IMAGE};;P: baz" is inserted at <placement> and selected at the <select-position>
    And "new" is typed
    Then the text is <text>
  ...
  Scenario: Pasting text blocks between two text blocks
    ...
    And "P: fizz;;P: buzz" is inserted at "auto"
  ...
  Scenario Outline: Inserting text block with annotation
    ...
    Examples:
      ...
      | "P: "                  | the caret is put after "" | "start"         | "P: foo [@link:bar] baz"        |
      | "P: "                  | the caret is put after "" | "end"           | "P: foo [@link:bar] baz"        |
      | "P: existing"          | "is" is selected          | "start"         | "P: exfoo [@link:bar] bazting"  |
      | "P: existing"          | "is" is selected          | "end"           | "P: exfoo [@link:bar] bazting"  |
      | "P: existing;;P: text" | "ingte" is selected       | "start"         | "P: existfoo [@link:bar] bazxt" |
      | "P: existing;;P: text" | "ingte" is selected       | "end"           | "P: existfoo [@link:bar] bazxt" |
