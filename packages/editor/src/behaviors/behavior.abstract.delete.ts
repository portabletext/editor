...
export const abstractDeleteBehaviors = [
  ...
  defineBehavior({
    ...
    guard: ({snapshot, event}) => {
      ...
      const previousBlockChildren = previousBlock.node.children
      if (!previousBlockChildren) {
        return false
      }
      ...
      const mergeContextData = {
        ...
        targetBlockTextLength: getTextBlockLength(snapshot.context, {
          children: previousBlockChildren,
        }),
        targetOriginalChildCount: previousBlockChildren.length,
      }
