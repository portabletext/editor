...
function resolveEscapeTargetBlock(
  snapshot: Parameters<typeof getAncestors>[0],
  path: Path,
): Path | undefined {
  ...
  for (const ancestor of getAncestors(snapshot, path)) {
    if (!isObjectNode(snapshot.context, ancestor.node)) {
      continue
    }
    ...
    if (snapshot.context.containers.has(scopedName)) {
  ...
  for (let i = 0; i < containerAncestors.length; i++) {
    ...
    const outerContainer = snapshot.context.containers.get(outerScopedName)
    if (!outerContainer) {
      continue
    }
