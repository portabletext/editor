...
function textSnapshot(key: string, text: string): BlockTextSnapshot {
  return {_key: key, text, isTextBlock: true}
}

function objectSnapshot(key: string): BlockTextSnapshot {
  return {_key: key, text: '', isTextBlock: false}
}

describe('detectChange', () => {
  describe('noop', () => {
    test('returns noop for two empty arrays', () => {
      expect(detectChange([], [])).toEqual({type: 'noop'})
    })

    test('returns noop when text content is identical', () => {
      const oldBlocks = [createTextBlock('block1', 'Hello world')]
      const newBlocks = [textSnapshot('block1', 'Hello world')]
      expect(detectChange(oldBlocks, newBlocks)).toEqual({
        type: 'noop',
      })
    })

    test('returns noop for multiple identical blocks', () => {
      const oldBlocks = [
      ...
      const newBlocks = [
        textSnapshot('block1', 'First'),
        textSnapshot('block2', 'Second'),
        textSnapshot('block3', 'Third'),
      ]
      expect(detectChange(oldBlocks, newBlocks)).toEqual({
        type: 'noop',
      })
  ...
  describe('text.insert', () => {
    test('detects single character insertion at end', () => {
      ...
      const newBlocks = [textSnapshot('block1', 'Hello!')]
      expect(detectChange(oldBlocks, newBlocks)).toEqual({
        type: 'text.insert',
        blockKey: 'block1',
        spanKey: 'block1-span',
    ...
    test('detects word insertion', () => {
      const oldBlocks = [createTextBlock('block1', 'Hello world')]
      const newBlocks = [textSnapshot('block1', 'Hello beautiful world')]
      expect(detectChange(oldBlocks, newBlocks)).toEqual({
        type: 'text.insert',
        blockKey: 'block1',
        spanKey: 'block1-span',
  ...
  describe('text.delete', () => {
    ...
    test('detects single character deletion at beginning', () => {
      const oldBlocks = [createTextBlock('block1', 'Hello')]
      const newBlocks = [textSnapshot('block1', 'ello')]
      expect(detectChange(oldBlocks, newBlocks)).toEqual({
        type: 'text.delete',
        blockKey: 'block1',
        spanKey: 'block1-span',
    ...
    test('detects multi-character deletion in middle', () => {
      const oldBlocks = [createTextBlock('block1', 'Hello world')]
      const newBlocks = [textSnapshot('block1', 'Held')]
      expect(detectChange(oldBlocks, newBlocks)).toEqual({
        type: 'text.delete',
        blockKey: 'block1',
        spanKey: 'block1-span',
    ...
    test('detects deletion to empty', () => {
      const oldBlocks = [createTextBlock('block1', 'Hello')]
      const newBlocks = [textSnapshot('block1', '')]
      expect(detectChange(oldBlocks, newBlocks)).toEqual({
        type: 'text.delete',
        blockKey: 'block1',
        spanKey: 'block1-span',
  ...
  describe('text.replace', () => {
    test('detects single character replacement', () => {
      const oldBlocks = [createTextBlock('block1', 'Hello')]
      const newBlocks = [textSnapshot('block1', 'Hallo')]
      expect(detectChange(oldBlocks, newBlocks)).toEqual({
        type: 'text.replace',
        blockKey: 'block1',
        spanKey: 'block1-span',
    ...
    test('detects word replacement via autocorrect (minimal diff)', () => {
      ...
      const newBlocks = [textSnapshot('block1', 'the quick')]
      expect(detectChange(oldBlocks, newBlocks)).toEqual({
        type: 'text.replace',
        blockKey: 'block1',
        spanKey: 'block1-span',
    ...
    test('detects full word replacement with different length', () => {
      const oldBlocks = [createTextBlock('block1', 'Hello world')]
      const newBlocks = [textSnapshot('block1', 'Hello universe')]
      expect(detectChange(oldBlocks, newBlocks)).toEqual({
        type: 'text.replace',
        blockKey: 'block1',
        spanKey: 'block1-span',
    ...
    test('detects IME composition replacement', () => {
      ...
      const newBlocks = [textSnapshot('block1', 'テスト書')]
      expect(detectChange(oldBlocks, newBlocks)).toEqual({
        type: 'text.replace',
        blockKey: 'block1',
        spanKey: 'block1-span',
    ...
    test('detects complete word swap', () => {
      ...
      const newBlocks = [textSnapshot('block1', 'dog')]
      expect(detectChange(oldBlocks, newBlocks)).toEqual({
        type: 'text.replace',
        blockKey: 'block1',
        spanKey: 'block1-span',
  ...
  describe('block.split', () => {
    test('detects split in the middle of text', () => {
      const oldBlocks = [createTextBlock('block1', 'Hello world')]
      const newBlocks = [
        textSnapshot('block1', 'Hello '),
        textSnapshot('block2', 'world'),
      ]
      expect(detectChange(oldBlocks, newBlocks)).toEqual({
        type: 'block.split',
    ...
    test('detects split of empty block (Enter on empty line)', () => {
      const oldBlocks = [createTextBlock('block1', '')]
      const newBlocks = [textSnapshot('block1', ''), textSnapshot('block2', '')]
      expect(detectChange(oldBlocks, newBlocks)).toEqual({
        type: 'block.split',
        originalBlockKey: 'block1',
        newBlockKey: 'block2',
  ...
  describe('block.merge', () => {
    ...
    test('detects merge when first block is empty', () => {
      ...
      const newBlocks = [textSnapshot('block2', 'Hello')]
    ...
    test('detects merge with multiple blocks present', () => {
      ...
      const newBlocks = [
        textSnapshot('block1', 'First'),
        textSnapshot('block2', 'Hello world'),
        textSnapshot('block4', 'Last'),
      ]
      expect(detectChange(oldBlocks, newBlocks)).toEqual({
        type: 'block.merge',
        survivingBlockKey: 'block2',
  ...
  describe('block.insert', () => {
    ...
    test('detects text block insertion that does not match split pattern', () => {
      ...
      const newBlocks = [
        textSnapshot('block1', 'Hello'),
        textSnapshot('block2', 'Something completely different'),
      ]
  ...
  describe('multi-span blocks', () => {
    test('detects text insertion across spans', () => {
      ...
      // The insertion offset (6) falls on the span1/span2 boundary in the old
      // block, so `findSpanKeyAtOffset` returns the next span (span2).
      expect(detectChange(oldBlocks, newBlocks)).toEqual({
        type: 'text.insert',
        blockKey: 'block1',
        spanKey: 'span2',
  ...
  describe('edge cases', () => {
    test('handles block objects (no children) without crashing', () => {
      ...
      const newBlocks = [objectSnapshot('block1')]
      expect(detectChange(oldBlocks, newBlocks)).toEqual({
        type: 'noop',
      })
    })
    ...
    test('handles Unicode text correctly', () => {
      ...
      const newBlocks = [textSnapshot('block1', '日本語のテスト')]
      expect(detectChange(oldBlocks, newBlocks)).toEqual({
        type: 'text.insert',
        blockKey: 'block1',
        spanKey: 'block1-span',
