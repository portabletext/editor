...
import {getSelection, isTrackedMutation} from '../engine/dom/utils/dom'
...
import type {DebouncedFunc} from '../engine/react/utils/debounce'
...
export class InputManager {
  ...
  handleDOMBeforeInput = (event: InputEvent): void => {
    ...
    if (this.isComposing && inputType === 'insertText' && event.cancelable) {
      ...
      if (savedSelection) {
        try {
          if (
            !this.editor.snapshot.context.selection ||
            !rangeEquals(this.editor.snapshot.context.selection, savedSelection)
          ) {
            this.editor.select(savedSelection)
          }
        } catch {
      ...
      if (!savedSelection && !this.editor.isNodeMapDirty) {
        ...
        if (nativeTargetRange) {
          try {
            const targetRange = DOMEditor.toEditorSelection(
              this.editor,
              nativeTargetRange,
            ...
            if (
              ...
              this.editor.select(targetRange)
            }
          } catch {
    ...
    if (
      ...
      if (text) {
        ...
        if (nativeTargetRange) {
          ...
          this.selectDOMSelectionFallback()
        }
  ...
  handleCompositionStart = (_event: CompositionEvent): void => {
    ...
    this.compositionStartSelection = this.editor.snapshot.context.selection
      ? {
          anchor: {...this.editor.snapshot.context.selection.anchor},
          focus: {...this.editor.snapshot.context.selection.focus},
        }
      : null
  ...
  handleCompositionEnd = (event: CompositionEvent): void => {
    ...
    const isCrossBoundary =
      ...
      !isCollapsedRange(savedSelection) &&
  ...
  handleKeyDown = (_event: KeyboardEvent): void => {
    ...
    // element; the original workaround hid the placeholder element here. The
    // engine no longer exposes `editor.domPlaceholderElement` (the placeholder
    // is rendered outside the engine), so the workaround has no element to
    // target and this is a no-op until the placeholder element is re-exposed.
  }
  ...
  private snapshotBlocks(): PortableTextBlock[] {
    return [...(this.editor.snapshot.context.value ?? [])]
  }

  private getCurrentCursorOffset(): number | undefined {
    const {selection} = this.editor.snapshot.context
  ...
  private applySelectionHint(hint: {blockKey: string; offset: number}): void {
    ...
    const block = (this.editor.snapshot.context.value ?? [])[blockIndex]
    ...
    try {
      ...
      if (
        !this.editor.snapshot.context.selection ||
        !rangeEquals(this.editor.snapshot.context.selection, range)
      ) {
  ...
  private tryFastPath(event: InputEvent): boolean {
    ...
    if (deleteEvent) {
      ...
      if (
        ...
        try {
          const root = DOMEditor.findDocumentOrShadowRoot(this.editor)
          ...
          if (domSelection && !domSelection.isCollapsed) {
            ...
            if (nativeTargetRange) {
              ...
              if (targetRange && isExpandedRange(targetRange)) {
                event.preventDefault()
                this.editor.select(targetRange)
  ...
  // Synthetic `beforeinput` events dispatched by browser extensions like
  // Grammarly carry no target ranges; they set the DOM selection on the range
  // to operate on instead. The `onDOMSelectionChange` flush in
  // `handleDOMBeforeInput` normally syncs the editor selection to that DOM
  // selection, but inside an iframe it can't: accepting an extension
  // suggestion moves focus off the editable, so the flush bails on its focus
  // check and the editor selection stays stale. Fall back to the DOM selection
  // so the input lands on the targeted range, not the stale cursor.
  private selectDOMSelectionFallback(): void {
    if (this.editor.isNodeMapDirty) {
      return
    }

    const domSelection = getSelection(
      DOMEditor.findDocumentOrShadowRoot(this.editor),
    )
    if (!domSelection || domSelection.rangeCount === 0) {
      return
    }

    const targetRange = DOMEditor.toEditorSelection(this.editor, domSelection, {
      exactMatch: false,
      // The DOM selection fallback is not guaranteed to be a valid editor
      // range, so don't throw on it.
      suppressThrow: true,
    })
    ...
    if (
      targetRange &&
      (!this.editor.snapshot.context.selection ||
        !rangeEquals(this.editor.snapshot.context.selection, targetRange))
    ) {
      this.editor.select(targetRange)
    }
  }
...
function isCrossBoundarySelection(selection: Range): boolean {
  return !pathEquals(selection.anchor.path, selection.focus.path)
}
