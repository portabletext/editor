  ...
  const handleCopy = useCallback(
    (event: ClipboardEvent<HTMLDivElement>): void | ReactEditor => {
      if (onCopy) {
        ...
        if (!position) {
          console.error('Could not find position for copy event')
          return
        }
  ...
  const handleCut = useCallback(
    (event: ClipboardEvent<HTMLDivElement>) => {
      if (onCut) {
        ...
        if (!position) {
          console.error('Could not find position for cut event')
          return
        }
        ...
        Promise.resolve(onPasteResult)
          .then((result) => {
            ...
            if (!result || !result.insert) {
              ...
              if (!position) {
                console.error('Could not find position for paste event')
                return
              }
  ...
  const handleDragEnter = useCallback(
    (event: React.DragEvent<HTMLDivElement>) => {
      ...
      if (!event.isDefaultPrevented() && !event.isPropagationStopped()) {
        ...
        if (!position) {
          console.error('Could not find position for dragenter event')
          return
        }
  ...
  const handleDragOver = useCallback(
    (event: React.DragEvent<HTMLDivElement>) => {
      ...
      if (!event.isDefaultPrevented() && !event.isPropagationStopped()) {
        ...
        if (!position) {
          console.error('Could not find position for dragover event')
          return
        }
  ...
  const handleDrop = useCallback(
    (event: React.DragEvent<HTMLDivElement>) => {
      ...
      if (!event.isDefaultPrevented() && !event.isPropagationStopped()) {
        ...
        if (!position) {
          console.error('Could not find position for drop event')
          return
        }
