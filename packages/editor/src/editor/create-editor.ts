...
import {
  assertUniqueDecorationIds,
  createDecorationsRegistration,
...
export function createInternalEditor(config: EditorConfig): {
  ...
  let registeredDecorationsCount = 0
  ...
  const editor: Editor = {
    ...
    registerDecorations: (decorationsConfig) => {
      assertUniqueDecorationIds(decorationsConfig.decorations)

      const sourceKey = `registered-decorations-${registeredDecorationsCount++}`

      return createDecorationsRegistration({
        rangeDecorationsActor,
        sourceKey,
        decorations: decorationsConfig.decorations,
        onMapped: decorationsConfig.onMapped,
      })
    },
    send: (event) => {
      switch (event.type) {
  ...
  config.subscriptions.push(() => {
    ...
    const subscription = config.editorActor.subscribe((snapshot) => {
      ...
      if (readOnly !== previousReadOnly) {
        ...
        config.rangeDecorationsActor.send({
