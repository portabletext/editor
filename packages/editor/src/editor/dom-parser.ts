...
export interface ParseDOMOptions {
   ...
   * Required so that the parser can distinguish text blocks from
   ...
   *   node via `elementToNode` and cast it to a PortableTextBlock.
 ...
 *    and cast it to PT. This is O(1) per block.
 *
...
export function parseDOMElement(
  ...
  if (slateNode && !hasSubtreeMutations(element, mutatedNodes)) {
    return slateNode as PortableTextBlock
  }
...
function extractLeafText(leafElement: HTMLElement): string {
  ...
  if (stringElement) {
    ...
    // PTE's TextString component adds a trailing '\n' spacer to prevent
    // browser collapse of trailing newlines (when isLast && text ends with
    // '\n'). Strip this spacer so we recover the original PT text.
    if (text.endsWith('\n\n')) {
