...
type InternalPatchEvent = NamespaceEvent<PatchEvent, 'internal'> & {
  ...
  // The exact signature `subscriber.repair-journal.ts` uses to identify a
  // startup repair: normalization firing while a remote frame is on the
  // stack. Carried on the event so the mutation batcher can classify the
  // patch by what produced it instead of by the actor state at receipt,
  // which the deferred-events replay in `setup` decouples from emission.
