...
export type SelectionState = {
  /**
   * Serialized path of the focused leaf (span, inline object, or block object).
   * Set when the selection is collapsed and resolves to a leaf.
   */
  ...
  /**
   * Serialized path of the focused container (text block or editable container).
   * Set when the selection is collapsed and has a container ancestor.
   */
 ...
 * - `focusedLeafPath` is the focus path if it resolves to a leaf, otherwise
 *   the nearest ancestor leaf (handles void objects whose selection focus
 *   points to a synthetic text child).
...
export function getSelectionState(
  ...
  for (const {node, path} of getNodes(context, {
    from: selection.anchorPath,
    to: selection.focusPath,
  })) {
