 ...
 * an inbound value sync has settled, a held bulk with no editable-time
 * patch is superseded (the full snapshot either already contains its
 * repair or normalization has re-emitted a fresh repair bulk, which the
 * cull leaves alone) and is dropped instead of flushing. Only value syncs
 * cull: a remote patch batch is a delta that can change engine state
 * without superseding a held repair, so it must leave held bulks alone.
 *
...
export function createMutationBatcher({
  ...
  // either already contains a held repair or the pass re-minted it, which
  // is what makes dropping safe (a remote patch batch proves neither, so
  // it never culls). A bulk still tagged with an older generation held
  // nothing but a repair the engine re-derives from the state that just
  // settled, so it's dropped; the generation bump after the drop closes
  // this pass off, so a later patch sharing
