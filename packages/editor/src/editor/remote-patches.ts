...
export function setupRemotePatches({
  ...
  const handleBufferedRemotePatches = () => {
    ...
    if (changed) {
      // No held-repair cull here, unlike a value sync: patches are deltas,
      // not full snapshots, so a batch can change engine state without
      // superseding a held repair, and the engine's own tree is already
      // repaired, so normalization re-mints nothing. Culling here orphans
      // the store's only copy of the fix. A batch that genuinely
