...
export function RenderContainerChild(props: {
  ...
  const leafConfig = useSelector(editorActor, (s) =>
    lookupScopedConfig(s.context.leafConfigs, leafScopedName),
  )
...
function RenderLegacyObject(props: {
  ...
  const {attributes, callbacks, element, isInlineObject, path, schema} = props
  ...
  return (
    ...
    >
      ...
      <LegacyVoidSpacer isInline={false} />
      <div
...
function LegacyVoidSpacer({isInline}: {isInline: boolean}) {
  ...
  return (
    ...
    >
      <span data-slate-node="text">
        <span data-slate-leaf data-pt-leaf="">
          <span
            data-slate-zero-width="z"
            data-slate-length={0}
            data-pt-zero-width="z"
            data-pt-length={0}
          >
            {!IS_ANDROID ? '\uFEFF' : null}
          </span>
