...
type SelectionState = {
   ...
   * selection is collapsed). `null` if selection is expanded or there is no
   * selection.
   */
  focusedChildKey: string | null
  /**
   ...
   * `null` if selection is expanded or there is no selection.
   */
...
export function SelectionStateProvider({
  ...
  const selectionState = useSelector(
    ...
    (editorActorSnapshot) => {
      ...
      let focusedChildKey: string | null = null

      if (isCollapsed) {
        const focusSpan = getFocusSpan(snapshot)
        focusedChildKey = focusSpan?.node._key ?? null
      }
      ...
      const focusedBlockKey = isCollapsed ? (startBlockKey ?? null) : null
