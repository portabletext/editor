...
type SyncValueEvent =
  ...
  | {
      ...
      value: Array<PortableTextBlock> | undefined
      changed: boolean
      // Whether the pass walked every block. `false` when `syncBlock` hit
      // a block a human has to resolve and the walk broke off early: the
      // remaining blocks were never examined, so `echoedBlockKeys` below
      // can't speak for them, and the pass must not emit `inbound state
      // applied` (see 'emit inbound state applied' below).
      completed: boolean
...
export const syncMachine = setup({
  ...
  actions: {
    ...
    // drops them next.
    //
    // Fires on every settle that completed its walk, carrying the pass's
    // echoed-block-keys set: a block in that set still echoes its
    // pre-repair shape (the engine's repair journal matched it against a
    // `beforeShape` while the engine still holds the `afterShape`), so
    // the mutation batcher's `dropSupersededRepairs` keeps that block's
    // held repair instead of dropping it as superseded when nothing has
    // actually superseded it. Skipped entirely when the pass aborted on
    // an invalid block: the walk never reached the remaining blocks, so
    // it has no echoed-block-keys verdict for them, and the settle signal
    // must not authorize `dropSupersededRepairs` to treat their held
    // repairs as superseded when they were simply never examined.
    'emit inbound state applied': enqueueActions(({event, enqueue}) => {
      assertEvent(event, 'done syncing')
      if (!event.completed) {
        return
      }
