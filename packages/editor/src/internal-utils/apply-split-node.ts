...
function transformPointForSplit(
  ...
  if (Path.equals(splitPath, path)) {
    if (
      position < offset ||
      (position === offset && affinity === 'forward')
    ) {
      offset -= position
...
export function applySplitNode(
  ...
  for (const ref of Editor.pathRefs(editor)) {
    ...
    if (current) {
      ref.current = transformPathForSplit(current, path, position, ref.affinity ?? 'forward')
    }
  }
  for (const ref of Editor.pointRefs(editor)) {
    const current = ref.current
    if (current) {
      ref.current = transformPointForSplit(current, path, position, ref.affinity ?? 'forward')
    }
  }
  for (const ref of Editor.rangeRefs(editor)) {
    ...
    if (current) {
      ...
      const anchor = transformPointForSplit(current.anchor, path, position, anchorAffinity ?? 'forward')
      const focus = transformPointForSplit(current.focus, path, position, focusAffinity ?? 'forward')
      if (anchor && focus) {
  ...
  if (editor.selection) {
    const anchor = transformPointForSplit(editor.selection.anchor, path, position, 'forward')
    const focus = transformPointForSplit(editor.selection.focus, path, position, 'forward')
    if (anchor && focus) {
