  ...
  describe('performance', () => {
    ...
    describe('deep structures', () => {
      function buildDeepMap() {
        ...
        for (let c = 0; c < 100; c++) {
          for (let n = 0; n < 50; n++) {
            map.onInsertNode([c, 0, n], `child-${c}-${n}`)
          }
        }
        return map
      }

      test('build 5,100-entry nested map (100 containers x 50 children)', () => {
        const start = performance.now()
        const map = buildDeepMap()
        const elapsed = performance.now() - start

        expect(map.size).toBe(5100)
        expect(elapsed).toBeLessThan(200)
        expect(map.get(['container-0', 'child-0-0'])).toEqual([0, 0, 0])
        expect(map.get(['container-99', 'child-99-49'])).toEqual([99, 0, 49])
      })

      test('5,000 nested lookups under 50ms', () => {
        const map = buildDeepMap()

        const start = performance.now()
        for (let c = 0; c < 100; c++) {
          for (let n = 0; n < 50; n++) {
            map.get([`container-${c}`, `child-${c}-${n}`])
          }
        }
        const elapsed = performance.now() - start
      ...
      test('insert inside container shifts only nested siblings', () => {
        const map = buildDeepMap()

        const start = performance.now()
        map.onInsertNode([50, 0, 0], 'new-nested-block')
        const elapsed = performance.now() - start

        expect(elapsed).toBeLessThan(5)
        expect(map.get(['container-50', 'new-nested-block'])).toEqual([
          50, 0, 0,
        ])
        expect(map.get(['container-50', 'child-50-0'])).toEqual([50, 0, 1])
      })

      test('insert top-level shifts all 5,100 entries', () => {
        const map = buildDeepMap()

        const start = performance.now()
        map.onInsertNode([0], 'new-top-block')
        const elapsed = performance.now() - start

        expect(elapsed).toBeLessThan(20)
        expect(map.get(['new-top-block'])).toEqual([0])
        expect(map.get(['container-0'])).toEqual([1])
        expect(map.get(['container-0', 'child-0-0'])).toEqual([1, 0, 0])
      })
    })
