import type {InsertPatch, Patch} from '@portabletext/patches'
...
import type {Path, PathSegment} from '../engine/interfaces/path'
...
/**
 * Derive the primitive point-mapping steps a transaction's patches produce,
 * applying each patch to a working copy of the value as it goes: a
 ...
 * transaction (see `decorator-add-mid-span` in the wire catalogue, whose
 * second `diffMatchPatch` targets the span text left behind by the first).
 *
  ...
  for (const patch of patches) {
    switch (patch.type) {
      case 'diffMatchPatch': {
        ...
        const spanPath = patch.path.slice(0, -1) as Path
        ...
        // A fold candidate is only the shape a real span merge produces:
        // the absorbed span's text appended at the destination's current
        // end. An insert landing mid-span or at the start is local typing
        // or an unrelated edit, never a merge, however its text happens to
        // read.
        ...
        writeAt(workingCopy, patch.path as Path, newText)
        break
      }

      case 'set': {
        const propertyName = patch.path[patch.path.length - 1]
        const nodePath = patch.path.slice(0, -1) as Path
        ...
        if (propertyName === '_key') {
          ...
          pushStep({
            type: 'set.text',
            path: nodePath,
            text: typeof patch.value === 'string' ? patch.value : '',
          })
          ...
          pushStep({
            ...
            newChildren: patch.value,
          })
        }

        writeAt(workingCopy, patch.path as Path, patch.value)
        break
      }
      ...
      case 'unset': {
        ...
        if (isKeyedSegment(lastSegment)) {
          ...
          // top of whatever the container's own key does. It's identified
          // by its exact path, not just its key value: a rename recorded
          // against one node must never sweep in some other, unrelated
          // node that happens to carry a pre-existing key equal to that
          // same new value (`renamedKeyPaths` guards exactly this). An
          // ordinary (never-renamed) descendant stays untracked here:
          // nothing resolves its own move.
        ...
        removeAt(workingCopy, patch.path as Path)
        break
      }
      ...
      case 'insert': {
        ...
        const parentPath = patch.path.slice(0, -1) as Path
        for (const item of patch.items) {
          const itemKey = nodeKey(item)
          if (itemKey === undefined) {
            continue
          }
          const itemPath: Path = [...parentPath, {_key: itemKey}]
...
type NodeRemoval = {key: string; path: Path; slotIndex: number}
type NodeInsertion = {key: string; path: Path; slotIndex: number}
type NodeMove = {
  from: Path
  to: Path
...
function assembleSteps(
  ...
  // the container can still hold more
  // than what moved (the contract every `remove.node` step exists to
  // enforce), so its step survives, relocated after the last such
  // descendant move so a point reaches every move before it can be
  // nulled.
...
function pairMoves(
  ...
  // Rule (a): a brand-new node pairs with a matching removal that's
  // either a whole node (in either patch order, same as rule (b) below,
  // a block merge's insert can land before or after the unset it pairs
  // with) or, for a removal that only carved text out of a still-living
  // span, one that precedes the insertion. Node insertions settle first,
  // so a whole-node removal that's genuinely a two-hop chain (moved to a
  // new node, then folded from there into a surviving span) claims its
  // first hop before rule (b) even looks at the second.
  ...
  // Rule (b): text folded into an existing span pairs only with a
  // whole-node removal whose current text matches it exactly, in either
  // patch order.
  for (const insertion of insertionCandidates) {
 ...
 * carried forward through those or it double-counts them (see the
 * `review probe F3` test for the failure this prevents).
 */
function mapRemovalForward(
  interveningSteps: ReadonlyArray<Step>,
  from: {path: Path; offset: number; length: number},
): {path: Path; offset: number; length: number} {
  const start = mapPointThroughSteps(interveningSteps, {
    path: from.path,
    offset: from.offset,
  })
  const end = mapPointThroughSteps(interveningSteps, {
    path: from.path,
...
function representativeSpan(
  node: unknown,
  path: Path,
): {path: Path; text: string} | undefined {
 ...
 * that carry one. `move.text`, `move.node`, and `rekey` don't address a
 * single fixed path (that's the whole point of a move), so they're
 * outside `assembleSteps`'s supersession sweep by construction.
 */
function stepPath(step: Step): Path | undefined {
  switch (step.type) {
    case 'insert.text':
    case 'remove.text':
    case 'set.text':
...
function collectKeyedNodes(
  node: unknown,
  path: Path,
): Array<{key: string; path: Path}> {
  if (!isPlainObject(node)) {
    return []
  }

  const results: Array<{key: string; path: Path}> = []
