            ...
            textBlock.children.some((child, cIndex: number) => {
              if (typeof child !== 'object' || child === null) {
                resolution = {
                  ...
                  description: `Child at index '${cIndex}' in block ${describeBlockLocation(blk, index)} is not an object.`,
              ...
              if (!child._type) {
                resolution = {
                  ...
                  description: `Child ${describeChildLocation(child, cIndex)} in block ${describeBlockLocation(blk, index)} is missing '_type' property.`,
                  ...
                  i18n: {
                    ...
                    values: {
                      key: blk._key,
                      childKey: hasUsableKey(child._key) ? child._key : cIndex,
                    },
                  },
                }
                return true
              ...
              if (!validChildTypes.includes(child._type)) {
                resolution = {
                  ...
                  description: `Child ${describeChildLocation(child, cIndex)} in block ${describeBlockLocation(blk, index)} has invalid '_type' property (${child._type}).`,
                  action: 'Remove the object',
                  item: blk,
              ...
              if (
                ...
                resolution = {
                  ...
                  description: `Child ${describeChildLocation(child, cIndex)} in block ${describeBlockLocation(blk, index)} has missing or invalid text property!`,
...
/**
 * A block without a usable `_key` can only be pointed at by position;
 * `index` is the block's position in the full document (`baseIndex`
 * applied), not in the slice being validated.
 */
...
function describeChildLocation(child: {_key?: unknown}, index: number): string {
  return hasUsableKey(child._key)
    ? `with _key '${child._key}'`
    : `at index '${index}'`
}
