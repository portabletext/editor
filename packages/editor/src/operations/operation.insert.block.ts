...
import {childSegment, resolveSegmentIndex} from '../types/paths'
...
export function insertBlock(options: {
  ...
  const start = at ? Range.start(editor, at) : Editor.start(editor, [])
  const end = at ? Range.end(editor, at) : Editor.end(editor, [])
  ...
  if (placement === 'after') {
    insertNodeAt(editor, (Node.next(editor, endBlockPath, editor.schema) ?? endBlockPath), block, select)
    return
  }
  ...
  if (Range.isExpanded(at) && !editor.isTextBlock(block)) {
    ...
    const start = Range.start(editor, at)
    ...
    const isAtStartOfBlock =
      ...
      Point.equals(start, startOfBlock)
    ...
    if (atAfterDelete) {
      ...
      const insertPath: Path = isAtStartOfBlock
        ...
        : [resolveSegmentIndex(editor.children, atAfterDelete.anchor.path[0]!) + 1]
    ...
    if (!editor.isTextBlock(block) && atAfterDelete) {
      ...
      try {
        ...
        if (
          Element.isElement(potentiallyEmptyBlock, editor.schema) &&
  ...
  if (
    ...
    const selectionPoint = Range.start(editor, at)
    ...
    if (!isAtBlockStart && !isAtBlockEnd) {
      ...
      const childIndex = resolveSegmentIndex(currentBlock.children, childSegment(selectionPoint.path)!)
      ...
      const insertPath: Path = [resolveSegmentIndex(editor.children, blockPath[0]!) + 1]
      editor.apply({type: 'insert_node', path: insertPath, node: block})
  ...
  if (editor.isTextBlock(endBlock) && editor.isTextBlock(block)) {
    let selectionStartPoint = Range.start(editor, at)
    ...
    if (Range.isExpanded(at)) {
      ...
      if (
        newEndBlock &&
        newEndBlockPath &&
        (Element.isElement(newEndBlock, editor.schema) ||
    ...
    if (select === 'end') {
      const insertAt = editor.selection
        ? Range.end(editor, editor.selection)
    ...
    // Use selectionStartPoint (updated after any deletion) instead of stale Range.start(editor, at)
    ...
    if (!editor.isTextBlock(endBlock)) {
      ...
      insertNodeAt(editor, [resolveSegmentIndex(editor.children, endBlockPath[0]!) + 1], block, select)
      return
    }

    const endBlockStartPoint = Editor.start(editor, endBlockPath)
    const endBlockEndPoint = Editor.end(editor, endBlockPath)
    const selectionStartPoint = Range.start(editor, at)
    const selectionEndPoint = Range.end(editor, at)
    ...
    if (
      ...
      if (isEmptyTextBlock(context, endBlock)) {
        removeNodeAt(editor, (Node.next(editor, endBlockPath, editor.schema) ?? endBlockPath))
      }
      return
    }
    ...
    if (
      ...
      const nextPath: Path = [resolveSegmentIndex(editor.children, endBlockPath[0]!) + 1]
    ...
    if (
      ...
      for (let i = resolveSegmentIndex(endBlock.children, childSegment(end.path)!) - 1; i >= 0; i--) {
    ...
    if (
      ...
      const [start] = Range.edges(editor, at)

      // Remove nodes after start
      const blockNode = Node.get(editor, endBlockPath, editor.schema) as Element
      for (let i = blockNode.children.length - 1; i > resolveSegmentIndex(blockNode.children, childSegment(start.path)!); i--) {
        removeNodeAt(editor, [...endBlockPath, i])
      }
      ...
      if (select !== 'none') {
        setSelection(editor, (Node.next(editor, endBlockPath, editor.schema) ?? endBlockPath), select)
      }
      return
    }
    ...
    if (focusChild && editor.isTextSpan(focusChild)) {
      const startPoint = Range.start(editor, at)
      ...
      if (editor.isTextBlock(block)) {
        ...
        if (select === 'none') {
          ...
          setSelection(editor, [resolveSegmentIndex(editor.children, endBlockPath[0]!) + 1], 'start')
        }
      } else {
        ...
        if (
          ...
          currentPath = (Node.next(editor, currentPath, editor.schema) ?? currentPath)
          currentOffset = 0
        }
        ...
        const splitAtIndex =
          currentOffset > 0
          ? resolveSegmentIndex((Node.get(editor, blockPath, editor.schema) as Element).children, childSegment(currentPath)!) + 1
          : resolveSegmentIndex((Node.get(editor, blockPath, editor.schema) as Element).children, childSegment(currentPath)!)
        const blockToSplit = Node.get(editor, blockPath, editor.schema)

        if (
          splitAtIndex < (blockToSplit as Element).children.length &&
          Element.isElement(blockToSplit, editor.schema)
        ) {
        ...
        const insertPath: Path = currentFirstBlockPath
          ? [resolveSegmentIndex(editor.children, currentFirstBlockPath[0]!) + 1]
          : [resolveSegmentIndex(editor.children, blockPath[0]!) + 1]
        editor.apply({type: 'insert_node', path: insertPath, node: block})
...
function replaceEmptyTextBlock(
  ...
  removeNodeAt(editor, (Node.next(editor, blockPath, editor.schema) ?? blockPath))
...
function deleteSameBlockRange(
  ...
  const endChildIndex = resolveSegmentIndex(block.children, childSegment(end.path)!)
  for (let i = endChildIndex - 1; i > resolveSegmentIndex(block.children, childSegment(start.path)!); i--) {
    removeNodeAt(editor, [...blockPath, i])
  }

  // Remove from beginning of end node
  const newEndPath: Path = [...blockPath, resolveSegmentIndex((Node.get(editor, blockPath, editor.schema) as Element).children, childSegment(start.path)!) + 1]
...
function deleteCrossBlockRange(
  ...
  if (start.path.length > 1) {
    ...
    for (let i = startBlock.children.length - 1; i > resolveSegmentIndex(startBlock.children, childSegment(start.path)!); i--) {
      removeNodeAt(editor, [...startBlockPath, i])
    }
  }

  // Remove all blocks between start and end
  const endBlockIndex = resolveSegmentIndex(editor.children, end.path[0]!)
  const startBlockIndex = resolveSegmentIndex(editor.children, start.path[0]!)
  for (let i = endBlockIndex - 1; i > startBlockIndex; i--) {
  ...
  if (end.path.length > 1) {
    ...
    for (let i = 0; i < resolveSegmentIndex((Node.get(editor, adjustedEndBlockPath, editor.schema) as Element).children, childSegment(end.path)!); i++) {
...
function insertTextBlockFragment(
  ...
  if (!Element.isElement(block, editor.schema) || !editor.isTextBlock(block)) {
    return
  }
  ...
  const parentNode = Node.get(editor, Path.parent(at.path), editor.schema) as Element
  let insertIndex = resolveSegmentIndex(parentNode.children, at.path[at.path.length - 1]!) + (at.offset > 0 ? 1 : 0)
