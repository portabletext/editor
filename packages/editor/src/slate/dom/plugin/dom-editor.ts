...
export interface DOMEditorInterface {
  ...
  /**
   * Find the native DOM element from a Slate path.
   */
  toDOMNode: (editor: Editor, path: Path) => HTMLElement
...
export const DOMEditor: DOMEditorInterface = {
  blur: (editor) => {
    const el = DOMEditor.toDOMNode(editor, [])
  ...
  hasDOMNode: (editor, target, options = {}) => {
    ...
    const editorEl = DOMEditor.toDOMNode(editor, [])
  ...
  toDOMNode: (editor, path) => {
    ...
    const serializedPath = keyedPath ? serializePath(keyedPath) : undefined

    if (serializedPath) {
      const domNode = editorElement.querySelector(
    ...
    throw new Error(
      `Cannot resolve a DOM node from Slate path: ${serializedPath}`,
    )
  },

  toDOMPoint: (editor, point) => {
    const [node] = editorNode(editor, point.path)
    const el = DOMEditor.toDOMNode(editor, point.path)
