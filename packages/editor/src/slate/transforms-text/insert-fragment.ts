...
export const insertFragment: TextTransforms['insertFragment'] = (
  ...
  Editor.withoutNormalizing(editor, () => {
    ...
    const inlineElementMatch = Editor.above(editor, {
      at,
      match: (n) => editor.isElement(n) && Editor.isInline(editor, n),
      mode: 'highest',
      voids,
    })
    ...
    for (const entry of Node.nodes({children: fragment} as Node, {
      ...
      if (shouldInsert(entry)) {
        if (editor.isElement(node) && !editor.isInline(node)) {
          starting = false
          middles.push(node)
        } else if (starting) {
          starts.push(node)
    ...
    const inlineMatch = Array.from(
      Editor.nodes(editor, {
        at,
        match: (n) => editor.isText(n) || Editor.isInline(editor, n),
        mode: 'highest',
        voids,
      }),
    )[0]!
    ...
    Transforms.splitNodes(editor, {
      ...
      match: (n) =>
        splitBlock
          ...
          : editor.isText(n) || Editor.isInline(editor, n),
