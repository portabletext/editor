...
export interface PortableTextEditorEngine extends DOMEditor {
  ...
  /**
   * Called by the sync machine once a value sync pass has fully settled,
   * whether or not it changed anything, including any fresh repair
   * patches normalization emitted in response. The argument carries the
   * keys of the blocks the pass found still echoing their pre-repair
   * shape (a snapshot the host hasn't learned a repair from yet). The
   * mutation batcher installs this to drop held repair bulks whose block
   * isn't in that set and isn't otherwise protected. Only value syncs call
   * it: a remote patch batch is a delta and cannot prove a held repair
   * superseded.
   */
