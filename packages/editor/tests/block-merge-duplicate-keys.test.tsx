...
describe('Feature: block merge renames colliding keys', () => {
  ...
  test("Scenario: a remote receiver's caret follows the renamed, reinserted child through the merge", async () => {
    ...
    // The caret sat in kB's third span (" baz") at offset 3. That span
    // reappears, renamed to `k4`, as the merged block's last child, so the
    // caret follows it there at the same offset instead of dangling on the
    // now-deleted `kB`.
    await vi.waitFor(() => {
      expect(editor2.getSnapshot().context.selection).toEqual({
