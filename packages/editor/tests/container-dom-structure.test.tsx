    ...
    test('with render', async () => {
      ...
      await vi.waitFor(() => {
        ...
        expect(formatHTML(el!.innerHTML)).toEqual(
          formatHTML(
            [
              '<div data-slate-node="element" data-pt-container="" data-pt-path="[_key==&quot;k0&quot;]" class="pt-block pt-text-block pt-text-block-style-normal" data-block-key="k0" data-block-name="block" data-block-type="text" data-style="normal">',
              '  <div>',
              ...
              '      <span data-slate-leaf="true" data-pt-leaf="">',
              '        <span data-slate-string="true" data-pt-string="true">hello</span>',
              '      </span>',
              '    </span>',
              '  </div>',
              '</div>',
        ...
        expect(formatHTML(normalizeInnerHTML(el!.innerHTML))).toEqual(
          formatHTML(
            [
              ...
              '    <span data-pt-spacer="" style="height: 0px; color: transparent; outline: none; position: absolute;">',
              '      <span data-pt-leaf="">',
  ...
  describe('defineLeaf (inline object)', () => {
    ...
    test('on root text block (no defineContainer)', async () => {
      ...
      await vi.waitFor(() => {
        const el = document.querySelector('[data-slate-editor]')
        expect(el).not.toEqual(null)
        const rootBlock = el!.querySelector('[data-block-type="text"]')
        expect(rootBlock).not.toEqual(null)
        // Inline stock-ticker leaf render applies
        expect(rootBlock!.textContent).toContain('[stock-ticker]')
      })
    })
  })
    ...
    test('on root level (no defineContainer)', async () => {
      ...
      await vi.waitFor(() => {
        ...
        const slateBlocks = el!.querySelectorAll(
          '[data-slate-node="element"][data-pt-container=""][data-block-type="text"]',
        )
  ...
  describe('edge cases', () => {
    ...
    test('defineLeaf without defineContainer is a no-op on root text blocks', async () => {
      ...
      await vi.waitFor(() => {
        const el = document.querySelector('[data-slate-editor]')
        expect(el).not.toEqual(null)
        // Without a defineContainer, defineLeaf is a no-op: custom render not applied
