...
describe('range selectors — container awareness', () => {
  test('getSelectedBlocks returns container-internal blocks when selection is inside a container', async () => {
    ...
    expect(blocks.map((b) => b.node._key)).toEqual([
      line1Key,
      line2Key,
      line3Key,
    ])
    expect(blocks[0]?.path).toEqual([
      {_key: codeBlockKey},
      'lines',
      {_key: line1Key},
    ])
  ...
  test('getSelectedValue returns sliced blocks inside a container', async () => {
    const keyGenerator = createTestKeyGenerator()
    const codeBlockKey = keyGenerator()
    const line1Key = keyGenerator()
    const line1SpanKey = keyGenerator()
    ...
    expect(value).toEqual([
      {
        _type: 'block',
        _key: line1Key,
        children: [{_type: 'span', _key: line1SpanKey, text: 'llo', marks: []}],
        markDefs: [],
        style: 'normal',
      },
      {
        _type: 'block',
        _key: line2Key,
        children: [{_type: 'span', _key: line2SpanKey, text: 'wor', marks: []}],
        markDefs: [],
        style: 'normal',
      },
    ])
