...
export default class HtmlDeserializer {
  ...
  /**
   * Create a new serializer respecting a Portable Text schema
   *
   * @param schema - Schema from @portabletext/schema
  ...
  deserializeElement = (element: Node): TypedObject | TypedObject[] => {
    ...
    for (let i = 0; i < this.rules.length; i++) {
      ...
      // Set newlines on spans originating from a block element within a blockquote
      if (
        ret &&
        !Array.isArray(ret) &&
        isMinimalBlock(ret) &&
