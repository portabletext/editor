...
describe(applyMarkdownEdit.name, () => {
  ...
  test('inserting a new link keeps the sibling annotated spans and their keys', () => {
    const keyGenerator = createTestKeyGenerator()
    const stored = [
      {
        _type: 'block',
        ...
        style: 'normal',
        markDefs: [
          {_type: 'link', _key: 'la', href: 'https://a.test'},
          {_type: 'link', _key: 'lb', href: 'https://b.test'},
        ],
        children: [
          {_type: 'span', _key: 's1', text: 'alpha', marks: ['la']},
          {_type: 'span', _key: 's2', text: ' mid ', marks: []},
          {_type: 'span', _key: 's3', text: 'beta', marks: ['lb']},
        ],
      },
    ]
    const markdown = portableTextToMarkdown(structuredClone(stored))
    expect(
      applyMarkdownEdit(stored, `[new](https://c.test) ${markdown}`, {
        deserialize: {keyGenerator},
      }),
    ).toEqual([
      {
        ...
        style: 'normal',
        markDefs: [
          {_type: 'link', _key: 'k1', href: 'https://c.test'},
          {_type: 'link', _key: 'la', href: 'https://a.test'},
          {_type: 'link', _key: 'lb', href: 'https://b.test'},
        ],
        children: [
          {_type: 'span', _key: 'k2', text: 'new', marks: ['k1']},
  ...
  test('an empty run reinserts next to the adopted anchor, not a pasted duplicate', () => {
    const keyGenerator = createTestKeyGenerator()
    const stored = [
      block('b1', 's1', 'alpha'),
    ...
    // paste a copy of the fence at the top; the original stays in place
    const edited = `${fence}\n\nalpha\n\n${fence}`
    expect(
      applyMarkdownEdit(stored, edited, {deserialize: {keyGenerator}}),
    ).toEqual([
      {_type: 'product', _key: 'k2', sku: 'abc'},
      block('b1', 's1', 'alpha'),
      {_type: 'product', _key: 'p1', sku: 'abc'},
      block('e1', 'es1', ''),
    ])
