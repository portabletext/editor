...
describe(portableTextToMarkdown.name, () => {
  ...
  describe('degradation report', () => {
    ...
    test('nested anchoring: an unknown annotation inside a table cell reports the table block, not the cell', () => {
      const keyGenerator = createTestKeyGenerator()
      const onDegradation =
        vi.fn<(report: SerializeDegradationReport) => void>()
      const markDefKey = keyGenerator()
    ...
    test('cap: a group with six occurrences lists five snippets', () => {
      ...
      expect(onDegradation).toHaveBeenCalledTimes(1)
      expect(onDegradation.mock.calls[0]![0]!.degradations).toHaveLength(6)
