/**
 * Keep in sync with `isDeepEqual` in `@portabletext/editor`'s
 * `src/internal-utils/equality.ts` (the generic half only; this plugin
 * never needs that file's Portable-Text-specific comparisons). Duplicated
 * because the core file isn't exported: this layer's `current` stability
 * check and its `update()` diffing both need value equality on plain
 * `{id, range}`-shaped data.
 *
 * More or less copied from Remeda
 * (https://github.com/remeda/remeda/blob/main/packages/remeda/src/isDeepEqual.ts)
 */
