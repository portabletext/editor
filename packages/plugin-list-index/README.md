...
function MyTextBlock(props: TextBlockRenderProps) {
  ...
  // 1-based position within the list, or `undefined` when the block
  // is not a list item
}
```

Typical use: custom text-block renders (`defineTextBlock`) that need to render numbered list markers, since the engine's default list-item wrapping (and the index it computes) does not apply to custom renders.
