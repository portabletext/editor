...
/**
 * `dropSupersededRepairs` (in the mutation batcher) only runs on a
 * completed `update value` pass; a `patches`-only reconciliation like
 * `applySync`'s confirmed repair or `'apply remote patches'` never gives it
 * that chance, so a read-only intake repair a newer store state has already
 * superseded stays queued and flushes stale on the editable flip. Re-entering
 * through `update value` with the snapshot just applied hands the pass that
 * judgment: the tree already matches (this reconciliation just made it so),
 * so the pass settles without emitting anything, but its completion still
 * lets the batcher cull a superseded repair. Scoped to read-only: once
 * editable, no repair is held this way, so the extra full-tree walk buys
 * nothing.
 */
function judgeHeldRepairsAgainst(editor: Editor, value: PortableTextBlock[]) {
  if (editor.getSnapshot().context.readOnly) {
...
function applySync({
  ...
  const timer = setTimeout(() => {
    ...
    if (diffValue(valueAfterPatches, latestRemote).length > 0) {
      ...
      judgeHeldRepairsAgainst(editor, latestRemote)
    }
  ...
  actions: {
    ...
    'apply remote patches': ({context, event}) => {
      ...
      queueMicrotask(() => {
        ...
        if (remoteValue) {
          judgeHeldRepairsAgainst(context.editor, remoteValue)
        }
      })
    },
  },
